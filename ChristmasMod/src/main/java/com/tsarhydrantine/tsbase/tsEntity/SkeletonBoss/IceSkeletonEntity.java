...
public class IceSkeletonEntity extends AbstractSkeletonEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new IceSkeletonEntity.SkeleAttackGoal(this));
	...
	static class GigaSnowballAttackGoal extends Goal {
		...
		public void tick() {
			...
			if (livingentity.getDistanceSq(this.iceSkeleEntity) < 4096.0D
					&& livingentity.getDistanceSq(this.iceSkeleEntity) > 256.0D
					&& this.iceSkeleEntity.canEntityBeSeen(livingentity)) {
				...
				if (this.attackTimer == 80) {
					...
					fireballentity.setPosition(this.iceSkeleEntity.getPosX() + vector3d.x * 4.0D,
							this.iceSkeleEntity.getPosYHeight(0.5D) + 0.5D,
							fireballentity.getPosZ() + vector3d.z * 4.0D);
